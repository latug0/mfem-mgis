...
void NewtonSolver::Mult(const Vector &b, Vector &x) const
{
   ...
   PROFILER_START(5A.1_NewtonProcessNewState);
   ProcessNewState(x);
   PROFILER_END();

   PROFILER_START(5A.2_NewtonFirstIteration);
   if (!iterative_mode)
   {
      x = 0.0;
   }
   ...
   for (it = 0; true; it++)
   {
      PROFILER_START(5A.3_NewtonIterMonitor);
      ...
      PROFILER_START(5A.4_NewtonIterGetGradient);
      ...
      PROFILER_END();
      PROFILER_START(5A.5_NewtonIterFirstMult);
      ...
      PROFILER_START(5A.6_NewtonIterSecondMult);
      ProcessNewState(x);
